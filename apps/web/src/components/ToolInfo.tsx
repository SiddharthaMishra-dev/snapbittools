...
export default function ToolInfo({ title, description, features, steps, privacyInfo, faqs }: ToolInfoProps) {
    ...
    return (
        <motion.div
            ...
            initial="hidden"
            animate="visible"
            className="w-full max-w-7xl mx-auto mt-2 space-y-16 pb-16"
        >
            ...
            {/* How it Works */}
            {/* <motion.div variants={itemVariants} className="bg-gray-800/30 border border-gray-700/50 rounded-2xl p-8 md:p-12">
                <div className="flex items-center space-x-3 mb-8">
                    <div className="w-10 h-10 bg-brand-primary rounded-full flex items-center justify-center text-white font-bold">?</div>
                ...
                <motion.div variants={containerVariants} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                    {steps.map((step, index) => (
                        <motion.div key={index} variants={featureVariants} className="relative">
